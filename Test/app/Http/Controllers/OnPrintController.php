...
class OnPrintController extends Controller
{
    ...
    public function print_change_status(Request $request)
    {
        ...
        if($status == 'Processing')
        {
            ...
            $notification_message = 'Print Vendor has started processing Order no: '.$order_id;
        ...
        elseif($status == 'Print Done')
        {
            ...
            // Inserting Notification for Admin
            $notification_message = 'Print Vendor has Completed Printing for Order no: '.$order_id;
            $notification_code = 4.2;
            Helper::insert_notification(-1,$notification_message,$notification_code);
        ...
        if($status == 'Print Complete and Received')
        {
            ...
            $notification_message = "Your Order no : ".$order_id." has been Prepared and On the way for Shipping. Thanks For Choosing AmarID.xyz";
            $notification_code = 4.3;
            Helper::insert_notification($user_id,$notification_message,$notification_code);



            // Inserting notification for Print Vendor || Admin Has received the Order
            $notification_message = 'Admin has received Order No : '.$order_id;
            Helper::insert_notification($assign_vendor->print_vendor_id,$notification_message,$notification_code);
