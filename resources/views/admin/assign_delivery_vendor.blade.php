                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="d-md-flex align-items-center">
                            <div>
                                <h1 class="card-title text-primary mb-4">Assign Delivery Vendor</h1>
                                @if($data['found'])
                                    <h5 class="card-subtitle text-success">Order Status : Print Done</h5>
                                @else
                                    <h5 class="card-subtitle text-danger">No Order of status : Print Done</h5>
                                @endif
                            </div>
                        </div>
                        <!-- title -->
                            ...
                            <table class="table v-middle  table-hover">
                                ...
                                @foreach($data['data'] as $all)
                                    <tr>
                                        ...
                                        @else
                                            <td>
                                                <img src="{{url('storage/'.$all->orderUrl.'/resize_front.jpg')}}" alt="" style="height: 40px;width: 60px;box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);">
                                            </td>
                                            <td>
                                                <img src="{{url('storage/'.$all->orderUrl.'/resize_back.jpg')}}" alt="" style="height: 40px;width: 60px;box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2), 0 6px 20px 0 rgba(0,0,0,0.19);">
                                            </td>
                                        @endif
                                        <td>
                                            <button type="submit" class="btn btn-sm btn-primary my-auto" data-toggle="modal" data-target="#vendor_modal" data-order_id="{{$all->orderID}}" >Assign Delivery Vendor</button>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-primary font-weight-bold text-center" id="vendor_modalLongTitle">Assign Delivery Vendor</h5>
                ...
                <div class="modal-body">
                    @if($data['vendor_found'])
                        <form method="POST" action="/select_delivery_vendor" >
                            @csrf
